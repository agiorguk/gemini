...
|Purpose and meaning |To enable data quality to be reported on a subset of the resource
...
*  a dataset series, set quality scope to 'series'

If using a Data Quality element to describe something other than the whole dataset or series, then provide additional DQ_DataQuality elements with their Scope set appropriately
# end::dataset[]
# tag::service[]
# end::service[]
...
|INSPIRE only allows a maximum of one Data quality element, so only one scope -
the whole resource
# end::dataset[]
# tag::service[]
|INSPIRE Metadata
